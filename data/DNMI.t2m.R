...
load("eof_DNMI_t2m.Rdata")
  ...
  DNMI.t2m  <- list(dat=xxx,lon=eof$lon,lat=eof$lat,tim=eof$tim,lev=NULL,
                       ...
                       attributes=eof$attributes,filename="DNMI.t2m")
  class(DNMI.t2m) <- class(eof)[-1]
rm(xxx,climxxx,i)
